...
def list_loads(current_user: User = Depends(require_user_department("fuel")), db: Session = Depends(get_db)):
...
def update_load(load_id: int, payload: LoadUpdate, current_user: User = Depends(require_user_department("fuel")), db: Session = Depends(get_db)):
...
def delete_load(load_id: int, current_user: User = Depends(require_user_department("fuel")), db: Session = Depends(get_db)):
    load = db.scalar(select(Load).where(Load.id == load_id, Load.user_id == current_user.id))
    if not load:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Load not found")
